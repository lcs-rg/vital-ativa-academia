...
class SolicitacaoMatriculaService {
  ...
  sanitize(data) { return { nome: data.nome?.trim(), email: data.email?.trim().toLowerCase(), telefone: data.telefone?.trim(), cep: data.cep?.trim() || null, logradouro: data.logradouro?.trim() || null, bairro: data.bairro?.trim() || null, cidade: data.cidade?.trim() || null, estado: data.estado?.trim() || null, plano_interesse: data.plano_interesse?.trim(), objetivo: data.objetivo?.trim() || null, status: 'pendente' }; }
