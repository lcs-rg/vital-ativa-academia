const service = require('../service/MatriculaService');
class MatriculaController {
  ...
  async create(req, res) { try { const id = await service.create(req.body); res.status(201).json({ id, message: 'Matrícula criada com sucesso' }); } catch (error) { res.status(error.status || 500).json({ error: error.message }); } }
}
