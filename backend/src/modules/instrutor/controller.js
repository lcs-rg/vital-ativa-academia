...
class InstrutorController {
  async getAll(req, res, next) {
    try { res.json(await service.getAll()); } catch (error) { next(error); }
  }
  async getById(req, res, next) {
    try { res.json(await service.getById(req.params.id)); } 
    catch (error) { if (error.message === 'Instrutor não encontrado') return res.status(404).json({ error: error.message }); next(error); }
  }
  ...
  async delete(req, res, next) {
    try { await service.delete(req.params.id); res.status(204).send(); }
    catch (error) { if (error.message === 'Instrutor não encontrado') return res.status(404).json({ error: error.message }); next(error); }
  }
}
